...
local whitelistedUsers = {
    "axxoryyy", "C4OS_Ld", "HoloGrindz", "EveryBaconisFriend", "Matias_XD2021", "12345AB801",
    "BloodPuppit2", "LK_HAZ2", "JayTonCost_24k", "singkomong04", "Mrbignewcoming3"
}
...
print("âœ… ID " .. player.Name .. " allowed. Loading the script...")
...
local function safeLoad(url, name)
    ...
    if not success then
        warn("âŒ Loading Failed: " .. name .. " - " .. tostring(result))
        return nil
    end

    print("âœ… " .. name .. " Successfully Loaded!")
    return result
end

-- URL of the main script to load
local mainScriptUrl = "https://raw.githubusercontent.com/SLHhub/Yamskrtt/refs/heads/main/TRYHARDERKIDDD" -- Ensure this URL is correct

-- Attempt to load the main script
local mainSuccess, mainResult = pcall(function()
    return loadstring(game:HttpGet(mainScriptUrl))() -- Use the URL directly here
end)

if not mainSuccess then
    warn("âŒ Error loading main script: " .. tostring(mainResult))
    player:Kick("âŒ Error: " .. tostring(mainResult))
    return
end
...
local MainSection = Tabs.Main:CreateSection("â¬Basic Controlsâ¬")
...
local RockSection = Tabs.AutoFarm:CreateSection("â¬Rock Farmâ¬")
local selectrock = ""
...
local Section = Tabs.AutoFarm:CreateSection("â¬Auto Farm Gymâ¬")
...
local Section = Tabs.Reb:CreateSection("â¬AutoRebirthâ¬")
...
local players = Tabs.Killer:CreateSection("â¬Auto Killâ¬")
local Players = game:GetService("Players")
